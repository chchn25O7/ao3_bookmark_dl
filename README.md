...
disrespecting the wishes of online people has never been easier, download your ao3 bookmarks and protect yourself against deleted fics
...
**The HTML option is different from ao3's inbuilt "Download -> HTML" link**
...
| `--delay` | `5.0` | Seconds to wait before every single request (any page load or file download) |
| `--limit N` | none | Only process the first N bookmarks (for testing) |
...
- This won't redownload fics that have been updated. You need to use `--force` flag to redownload all (sorry)